...
def _draw_page_two(c: canvas.Canvas, character: dict[str, Any]) -> None:
    ...
    lower_h = 118
    lower_gap = 8
    lower_w = (backstory_w - lower_gap) / 2
    right_x = 376
    right_w = PAGE_W - MARGIN - right_x
    bottom_y = 24
    bottom_h = 124
    bottom_gap = 10
    bottom_w = (PAGE_W - MARGIN * 2 - bottom_gap) / 2

    _draw_text_area(c, "Backstory", character.get("backstory"), backstory_x, backstory_y, backstory_w, 294, line_count=18)
    _draw_feature_area(
        c,
    ...
    _draw_text_area(c, "Aspetto", appearance_text, backstory_x, lower_y, lower_w, lower_h, line_count=7)
    _draw_text_area(
        c,
    ...
    _draw_text_area(c, "Note libere", notes, MARGIN, bottom_y, bottom_w, bottom_h, line_count=8)
    _draw_text_area(
        c,
        "Tesoro / zaino",
        character.get("treasure") or character.get("backpack") or character.get("zaino"),
        MARGIN + bottom_w + bottom_gap,
        bottom_y,
        bottom_w,
        bottom_h,
        line_count=8,
