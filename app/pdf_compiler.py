...
def _draw_page_one(c: canvas.Canvas, character: dict[str, Any]) -> None:
    ...
    _draw_combat(c, right_x, upper_y + 270, right_w, 250, character, hp)
    _draw_attacks(c, right_x, upper_y, right_w, 260, character.get("attacks", []))
    _draw_text_area(
        c,
        "Equipaggiamento",
...
def _draw_page_two(c: canvas.Canvas, character: dict[str, Any]) -> None:
    ...
    _draw_text_area(c, "Backstory", character.get("backstory"), MARGIN, 290, 340, 294, line_count=18)
    _draw_feature_area(
        c,
    ...
    _draw_text_area(c, "Aspetto", appearance_text, MARGIN, 160, 175, 118, line_count=7)
    _draw_text_area(c, "Alleati e organizzazioni", character.get("allies_and_organizations"), 207, 160, 180, 118, line_count=7)
    _draw_text_area(c, "Tesoro", character.get("treasure"), 399, 160, PAGE_W - MARGIN - 399, 118, line_count=7)

    notes = character.get("notes_for_unfilled_fields") or character.get("notes") or ""
    _draw_text_area(c, "Note libere", notes, MARGIN, 24, PAGE_W - MARGIN * 2, 124, line_count=8)
...
def _draw_header(c: canvas.Canvas, character: dict[str, Any]) -> None:
    ...
    y = 706
    name_w = 185
    _draw_field(c, "Nome personaggio", header.get("character_name"), MARGIN, y, name_w, 58, value_size=13)

    x = MARGIN + name_w + 8
    w = PAGE_W - MARGIN - x
    gap = 6
    col_w = (w - gap * 2) / 3
    _draw_field(c, "Classe e livello", header.get("class_and_level"), x, y + 30, col_w, 28)
    _draw_field(c, "Background", header.get("background"), x + col_w + gap, y + 30, col_w, 28)
    _draw_field(c, "Giocatore", header.get("player_name"), x + (col_w + gap) * 2, y + 30, col_w, 28)
    _draw_field(c, "Razza/specie", header.get("race_or_species"), x, y, col_w, 28)
    _draw_field(c, "Allineamento", header.get("alignment"), x + col_w + gap, y, col_w, 28)
    _draw_field(c, "XP", header.get("experience_points"), x + (col_w + gap) * 2, y, col_w, 28)
...
def _draw_text_area(
    ...
    *,
    ...
    lines = _wrap(text, max_chars)[:line_count]
    yy = content_top - 1
    for line in lines:
...
def _draw_feature_area(
    ...
    *,
    ...
    lines = _feature_lines(features, max_chars=max(18, int(w / 3.55)))[:line_count]
    c.setFillColor(INK)
    yy = content_top - 1
    for line in lines:
...
def _draw_abilities(c: canvas.Canvas, x: float, y: float, w: float, h: float, abilities: dict[str, Any]) -> None:
    ...
    for index, (label, key, short, aliases) in enumerate(ABILITY_DEFS):
        ...
        c.setStrokeColor(INK)
        c.setLineWidth(0.7)
        c.roundRect(x + 8, yy + 5, w - 16, row_h - 8, 5, stroke=1, fill=0)
        c.setFillColor(MID)
        c.setFont("Helvetica-Bold", 6.4)
        c.drawCentredString(x + w / 2, yy + row_h - 8, _text(label).upper())
        c.setFillColor(INK)
        c.setFont("Helvetica-Bold", 17)
        c.drawCentredString(x + w / 2, yy + 21, _text(mod))
        c.setFont("Helvetica", 8)
        c.drawCentredString(x + w / 2, yy + 9, _text(score))
...
def _draw_saving_throws(c: canvas.Canvas, x: float, y: float, w: float, h: float, character: dict[str, Any]) -> None:
    ...
    for index, (label, key, short, aliases) in enumerate(ABILITY_DEFS):
        ...
        proficient = _truthy(row.get("proficient") or row.get("competent") or row.get("proficiency")) if row else False
...
def _draw_skills(c: canvas.Canvas, x: float, y: float, w: float, h: float, character: dict[str, Any]) -> None:
    ...
    _draw_field(c, "Percezione passiva", passive, x + 8, y + 8, w - 16, 27, center=True)
...
def _draw_combat(
    ...
    stat_w = (w - 16 - gap * 2) / 3
    stat_y = y + h - 72
    _draw_field(c, "CA", character.get("armor_class"), x + 8, stat_y, stat_w, 43, value_size=13, center=True)
    _draw_field(c, "Iniziativa", character.get("initiative"), x + 8 + stat_w + gap, stat_y, stat_w, 43, value_size=13, center=True)
    _draw_field(c, "Velocita", character.get("speed"), x + 8 + (stat_w + gap) * 2, stat_y, stat_w, 43, value_size=11, center=True)

    _draw_field(c, "Bonus competenza", character.get("proficiency_bonus"), x + 8, stat_y - 34, stat_w, 28, center=True)
    _draw_field(c, "Ispirazione", character.get("inspiration"), x + 8 + stat_w + gap, stat_y - 34, stat_w, 28, center=True)
    _draw_field(c, "Dadi vita", character.get("hit_dice"), x + 8 + (stat_w + gap) * 2, stat_y - 34, stat_w, 28, center=True)

    _draw_field(c, "PF massimi", hp.get("maximum"), x + 8, y + 120, w - 16, 28, center=True)
    _draw_field(c, "PF attuali", hp.get("current"), x + 8, y + 58, (w - 21) / 2, 56, value_size=13, center=True)
    _draw_field(c, "PF temporanei", hp.get("temporary"), x + 13 + (w - 21) / 2, y + 58, (w - 21) / 2, 56, value_size=13, center=True)

    c.setFillColor(INK)
    c.setFont("Helvetica-Bold", 6.6)
    c.drawString(x + 11, y + 39, "TIRI SALVEZZA MORTE")
    c.setFont("Helvetica", 7)
    c.drawString(x + 11, y + 23, "Successi")
    c.drawString(x + 90, y + 23, "Fallimenti")
    _three_empty_circles(c, x + 56, y + 25)
    _three_empty_circles(c, x + 139, y + 25)
...
def _draw_attacks(c: canvas.Canvas, x: float, y: float, w: float, h: float, attacks: list[dict[str, Any]]) -> None:
    ...
    c.setFillColor(MID)
    c.setFont("Helvetica-Bold", 5.8)
    c.drawString(x + 8, top + 8, "NOME")
    c.drawString(x + 10 + name_w, top + 8, "BONUS")
    c.drawString(x + 12 + name_w + bonus_w, top + 8, "DANNO/TIPO")
    ...
    _draw_text_area(c, "Note attacchi / descrizione", notes, x + 8, y + 10, w - 16, 88, line_count=5)
...
def _draw_spell_block(
    ...
    if level > 0:
        ...
        c.setFillColor(MID)
        c.drawString(x + w - 86, y + h - 12, "SLOT")
        for idx in range(6):
            c.rect(x + w - 61 + idx * 9, y + h - 15, 6, 6, stroke=1, fill=0)
        if slots_total not in (None, ""):
            c.setFillColor(INK)
            c.setFont("Helvetica", 6)
            c.drawRightString(x + w - 66, y + h - 12, _text(slots_total))
...
def _row_bonus(row: dict[str, Any] | None, abilities: dict[str, Any], ability_key: str, proficiency_bonus: int | None) -> str:
    ...
    if row and proficiency_bonus is not None:
        ...
        elif _truthy(row.get("proficient") or row.get("competent") or row.get("proficiency")):
