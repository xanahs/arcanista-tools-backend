...
def _draw_abilities(c: canvas.Canvas, x: float, y: float, w: float, h: float, abilities: dict[str, Any]) -> None:
    ...
    for index, (label, key, short, aliases) in enumerate(ABILITY_DEFS):
        ...
        c.drawCentredString(mod_x + mod_w / 2, mod_y + 6.5, _text(mod))

        score_x = cell_x + cell_w - 29
        score_y = cell_y + 19.5
        c.setStrokeColor(INK)
        c.circle(score_x, score_y, 12, stroke=1, fill=0)
        c.setFont("Helvetica", 8)
        c.drawCentredString(score_x, score_y - 2.7, _text(score))
...
def _draw_skills(c: canvas.Canvas, x: float, y: float, w: float, h: float, character: dict[str, Any]) -> None:
    ...
    passive_w = 112
    _draw_field(c, "Percezione passiva", passive, x + (w - passive_w) / 2, y + 8, passive_w, 24, center=True)
...
def _draw_combat(
    ...
    stat_w = (w - 16 - gap * 2) / 3
    stat_y = y + h - 70
    _draw_field(c, "CA", character.get("armor_class"), x + 8, stat_y, stat_w, 40, value_size=13, center=True)
    _draw_field(c, "Iniziativa", character.get("initiative"), x + 8 + stat_w + gap, stat_y, stat_w, 40, value_size=13, center=True)
    _draw_field(c, "Velocita", character.get("speed"), x + 8 + (stat_w + gap) * 2, stat_y, stat_w, 40, value_size=10, center=True)

    secondary_y = stat_y - 35
    _draw_field(c, "Bonus comp.", character.get("proficiency_bonus"), x + 8, secondary_y, stat_w, 26, center=True)
    _draw_field(c, "Ispirazione", character.get("inspiration"), x + 8 + stat_w + gap, secondary_y, stat_w, 26, center=True)
    _draw_field(c, "Dadi vita", character.get("hit_dice"), x + 8 + (stat_w + gap) * 2, secondary_y, stat_w, 26, center=True)

    _draw_field(c, "PF massimi", hp.get("maximum"), x + 8, y + 118, w - 16, 22, center=True)
    _draw_field(c, "PF attuali", hp.get("current"), x + 8, y + 58, (w - 21) / 2, 50, value_size=13, center=True)
    _draw_field(c, "PF temporanei", hp.get("temporary"), x + 13 + (w - 21) / 2, y + 58, (w - 21) / 2, 50, value_size=13, center=True)
...
def _draw_spell_block(
    ...
    if level > 0:
        ...
        slot_count = _slot_count_for_spell_level(level, slots_total)
        slot_size = 6
        slot_gap = 4
        slots_w = slot_count * slot_size + max(0, slot_count - 1) * slot_gap
        start_x = x + w - 10 - slots_w
        for idx in range(slot_count):
            c.rect(start_x + idx * (slot_size + slot_gap), y + h - 15, slot_size, slot_size, stroke=1, fill=0)
...
def _slot_count_for_spell_level(level: int, slots_total: Any) -> int:
    default = SPELL_SLOT_MAX_BY_LEVEL.get(level, 0)
    explicit = _parse_int(slots_total)
    if explicit is None:
        return default
    return max(default, min(explicit, 8))
